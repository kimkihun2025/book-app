...
function removeBook(button) {
    ...
    // books 배열에서 도서 제거 (findIndex, splice 사용)
    const bookIdx = books.findIndex((book) => book.title === title);
    if (bookIdx !== -1) books.splice(bookIdx, 1);

    // rentals 배열에서 대여 상태 제거 (findIndex, splice 사용)
    const rentalIdx = rentals.findIndex(
        (rental) => rental.getStatus().title === title
    );
    if (rentalIdx !== -1) rentals.splice(rentalIdx, 1);
...
function toggleRental(button) {
    ...
    const rental = rentals.find((rental) => rental.getStatus().title === title);
    if (!rental) return;
